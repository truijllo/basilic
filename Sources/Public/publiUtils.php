...
switch($lg)
{
	...
	case "de":
	$txtPhd="PhD These";
	$txtHDR="Forschung �berwachung Diplom";
...
// ancienne ligne en dessous, s�curit� pour �viter d'afficher 2x
//      if($row[proceedings]!="")
...
//	$resu=sqlQuery("select  first,last,url from (authors A join publiauthors PA on A.id=PA.idAuthor) where PA.idPubli='$row[id]' order by rank");
	...
	switch($row['entry'])
	{
		// parametrages selon la classe d'entr�e
		case "Article":
	...
	switch($row['entry'])
	{
		...
		case "Article":
			echo "$row[journal]";
			if (!empty($row[publisher])) echo " $row[publisher]";	// rajout� le 25/04/2006
