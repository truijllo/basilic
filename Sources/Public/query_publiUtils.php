...
//      if($row[proceedings]!="")
    ...
    case "Article":
      JSPrintMsg($row[journal]);
      break;
      
    case "Book":
    case "InBook":
      JSPrintMsg( $row["publisher"]);
      break;
      
    case "PhdThesis":
      if ($row["type"] == "Habilitation &agrave; Diriger des Recherches") JSPrintMsg(" $txtHDR - ".$row["school"]);
      else JSPrintMsg(" $txtPhd - ".$row["school"]);
      break;

    case "MastersThesis":
      JSPrintMsg("$txtMaster".$row["school"]." ");
      break;
      
    case "TechReport":
      JSPrintMsg("$txtTechRep $row[number], $row[institution]");
      break;
      
    case "InCollection":
      JSPrintMsg("$row[booktitle], $row[publisher]");
      break;

    default:
      JSPrintMsg($row["entry"]);
      break;
    }
 
 if (!empty($row["volume"]))
	 JSPrintMsg(", Volume $row[volume]");
 if (!empty($row["number"]))
	JSPrintMsg(", Number $row[number]");
 if (!empty($row["pages"]))
	JSPrintMsg(", page $row[pages]");

  JSPrintMsg(" - $row[month] $row[year]\n");
}
...
function printAuthors($id)
{
  ...
  while ($aresult && $arow=mysql_fetch_array($aresult))
	...
	JSPrintMsg(", ");
      if (!empty($arow["url"]))
	JSPrintMsg("   <a href='$arow[url]'>$arow[first] $arow[last]</a>");
      else
	JSPrintMsg("   $arow[first] $arow[last]");
      $first = false;
    }
}
...
// thumbs for files
 ...
 while ($dresult && $drow=mysql_fetch_array($dresult))
 {
  ...
  if (isset($thumb))
    {
      JSPrintMsg(" <div class='thumb image'>\n");
      JSPrintMsg(" $thumb");
      JSPrintMsg(" </div>\n");
      JSPrintMsg(" <div class='ref'>\n");
    }
  else
//    echo " <div class='thumb'></div>\n";
    JSPrintMsg(" <div class='ref_nothumb'>\n");

  JSPrintMsg("  <div class='title'>\n   <a href=\"$path/\">\n    ".$row["title"]."\n   </a> &nbsp; \n"); 
  JSPrintMsg("$docs  </div>\n");

  // Search for authors
  JSPrintMsg("  <div class='authors'>\n");
  printAuthors($id);
  JSPrintMsg("\n  </div>\n");

  JSPrintMsg("  <div class='conf'>\n   ");
  printConference($row);
  JSPrintMsg("  </div>\n");
  JSPrintMsg(" </div>\n\n");
}
...
function printGalleryLine($row)
{
  ...
  JSPrintMsg(" <div class='imgMovies'>\n");
...
if($_SESSION["Affimg"]==0)
{
  ...
  while ($dresult && $drow=mysql_fetch_array($dresult))
      ...
      if (($type=="IMG") || ($type=="MOV"))
	{
	  ...
	  JSPrintMsg("  <div class='thumb $class'><div style='padding-top:$pt'><a href='$path/$add_path$src'>\n");
	  JSPrintMsg("  <img src='$path/.thumbs/$src.jpg' width='$w' height='$h' alt='$src $sizeText'/></a></div></div>\n");
	}
    }
   //ajouter pour supprimer les image
   }
...
function printBibTex($row,$bool)
{
  ...
  if($bool==1) JSPrintMsg("<pre class='bibtex'>\n$res}\n</pre>\n\n");
  return $res;
}
...
function printXML($row)
{
  ...
  $aresult = sqlQuery("SELECT first, last, url FROM authors, publiauthors WHERE authors.id = publiauthors.idAuthor AND publiauthors.idPubli=$row[id] ORDER BY rank");
  while ($arow = mysql_fetch_array($aresult))
    JSPrintMsg("   &lt;AUT_REF&gt;$arow[last] $arow[first]&lt;/AUT_REF&gt;\n");
  mysql_free_result ($aresult);
  JSPrintMsg("  &lt;/AUTEURS&gt;\n");
  
  JSPrintMsg("  &lt;TITR_REF&gt;".escapeXMLChars($row["title"])."&lt;/TITR_REF&gt;\n");
  JSPrintMsg("  &lt;ANN_PUB&gt;$row[year]&lt;/ANN_PUB&gt;\n");

  if ($row["number"] || $row["volume"] || $row["pages"] || $row["series"])
    {
      JSPrintMsg("  &lt;COLL_REF&gt;\n");
      if ($row["number"]) { JSPrintMsg("   &lt;NUM&gt;$row[number]&lt;/NUM&gt;\n"); }
      if ($row["series"]) { JSPrintMsg("   &lt;COL_LIB&gt;".escapeXMLChars($row[series])."&lt;/COL_LIB&gt;\n"); }

      if ($row["volume"]) { JSPrintMsg("   &lt;VOL&gt;$row[volume]&lt;/VOL&gt;\n"); }
      if ($row["pages"])  { JSPrintMsg("   &lt;PAG&gt;$row[pages]&lt;/PAG&gt;\n"); }
      JSPrintMsg("  &lt;/COLL_REF&gt;\n");
    }
   ...
   switch($row["entry"])
    ...
    case "Article":
      JSPrintMsg("  &lt;INFO_PUB&gt;\n");
      JSPrintMsg("   &lt;REVUE&gt;".escapeXMLChars($row["journal"])."&lt;/REVUE&gt;\n");
      JSPrintMsg("  &lt;/INFO_PUB&gt;\n");
      $typeP="ART";
      $typeS="ACL";
      break;
    ...
    case "InProceedings":
      JSPrintMsg("  &lt;INFO_COL&gt;\n");
      JSPrintMsg("   &lt;TEXT_COL&gt;".escapeXMLChars($row["booktitle"])."&lt;/TEXT_COL&gt;\n");
      JSPrintMsg("  &lt;/INFO_COL&gt;\n");
      $typeP="COL";
      $typeS="ACT";
      break;
    ...
    case "Book":
      JSPrintMsg("  &lt;INFO_OUV&gt;\n");
      JSPrintMsg("   &lt;EDIT&gt;".escapeXMLChars($row[publisher])."&lt;/EDIT&gt;\n");
      JSPrintMsg("  &lt;/INFO_OUV&gt;\n");
      $typeP="OUV";
      $typeS="RCH";
      break;
    ...
    case "MastersThesis":
      JSPrintMsg("  &lt;INFO_TRU&gt;\n");
      JSPrintMsg("   &lt;UNIV_TRU&gt;".escapeXMLChars($row[school])."&lt;/UNIV_TRU&gt;\n");
      JSPrintMsg("  &lt;/INFO_TRU&gt;\n");
      $typeP="TRU";
    ...
    case "TechReport":
      JSPrintMsg("  &lt;INFO_RAP&gt;\n");
      JSPrintMsg("   &lt;TEXT_RAP&gt;$row[number], ".escapeXMLChars($row[institution])."&lt;/TEXT_RAP&gt;\n");
      JSPrintMsg("  &lt;/INFO_RAP&gt;\n");
      $typeP="RAP";
      $typeS="REC";
      break;
    ...
    case "InBook":
      JSPrintMsg("  &lt;INFO_COV&gt;\n");
      if ($row["booktitle"]) JSPrintMsg("   &lt;TITR_OUV&gt;".escapeXMLChars($row[booktitle])."&lt;/TITR_OUV&gt;");
      if ($row["editor"])    JSPrintMsg("   &lt;EDIT_OUV&gt;".escapeXMLChars($row[editor])."&lt;/EDIT_OUV&gt;");
      if ($row["publisher"]) JSPrintMsg("   &lt;TEXT_OUV&gt;".escapeXMLChars($row[publisher])."&lt;/TEXT_OUV&gt;");
      JSPrintMsg("  &lt;/INFO_COV&gt;\n");
      $typeP="COV";
      break;
  ...
  JSPrintMsg("  &lt;THEM_SCI&gt;\n");
  JSPrintMsg("   &lt;CN_SCI&gt;\n    &lt;SEC_SCI&gt;7&lt;/SEC_SCI&gt;\n   &lt;/CN_SCI&gt;\n");
  if ($row["keywords"]) JSPrintMsg("   &lt;MC_LIB&gt;".escapeXMLChars($row[keywords])."&lt;/MC_LIB&gt;\n");
  if ($row["note"]) JSPrintMsg("   &lt;MC_LIB&gt;".escapeXMLChars($row[note])."&lt;/MC_LIB&gt;\n");
  JSPrintMsg("  &lt;/THEM_SCI&gt;\n");

  JSPrintMsg("  &lt;INFO_REF&gt;\n");
  // echo "   &lt;COD_UNIT&gt;UMRxxxx&lt;/COD_UNIT&gt;\n"; filled automatically
  JSPrintMsg("   &lt;COD_EQU&gt;ENSPS&lt;/COD_EQU&gt;\n");
  JSPrintMsg("   &lt;TYPE_P&gt;$typeP&lt;/TYPE_P&gt;\n");
  if (!empty($typeS)) JSPrintMsg("   &lt;TYPE_S&gt;$typeS&lt;/TYPE_S&gt;\n");
  JSPrintMsg("   &lt;TYPE_L&gt;$row[entry]&lt;/TYPE_L&gt;\n");
  JSPrintMsg("   &lt;STATUT&gt;PUB&lt;/STATUT&gt;\n");
  JSPrintMsg("   &lt;ANN_PROD&gt;$row[year]&lt;/ANN_PROD&gt;\n");
  JSPrintMsg("   &lt;ID_ORIG&gt;ENSPS-$row[bibTex]&lt;/ID_ORIG&gt;\n");
  JSPrintMsg("  &lt;/INFO_REF&gt;\n");

  JSPrintMsg("  &lt;INFO_URL&gt;\n");
  JSPrintMsg("   &lt;URL_TEXTE&gt;$local_server$public_path/$row[year]/$row[bibTex]&lt;/URL_TEXTE&gt;\n");
  JSPrintMsg("   &lt;URL_IMAGE&gt;$local_server$publix_path/$row[year]/$row[bibTex]&lt;/URL_IMAGE&gt;\n");
  JSPrintMsg("  &lt;/INFO_URL&gt;\n");
...
// thumbs for files
	 ...
	 while ($dresult && $drow=mysql_fetch_array($dresult))
	 {
	      ...
	      if ($type=="PDF") JSPrintMsg("  <a href=\"$path/$add_path$src\"><img class='doc' src='$local_server$images_path/pdf.png' width='19' height='19' alt='$src' /></a>");
	      if ($type=="PS")  JSPrintMsg("   <a href=\"$path/$add_path$src\"><img class='doc' src='$local_server$images_path/ps.png' width='19' height='19' alt='$src' /></a>");
	      if ($type=="TXT")  JSPrintMsg("   <a href=\"$path/$add_path$src\"><img class='doc' src='$local_server$images_path/txt.png' width='19' height='19' alt='$src' /></a>");
	 }
...
//	$resu=sqlQuery("select  first,last,url from (authors A join publiauthors PA on A.id=PA.idAuthor) where PA.idPubli='$row[id]' order by rank");
	...
	if($acti=="change")
	{
		...
		{
			case 1:
			JSPrintMsg("<table width=100%><tr align=left><td><b>Works and collaborations</td></tr> <tr height=2><td bgcolor=#003399></td></tr></table>");
			break;
			case 2:
			JSPrintMsg("<table width=100%><tr align=left><td><b>Peer-reviewed publications in International Journals</td></tr> <tr height=2><td bgcolor=#003399></td></tr></table>");
			break;
			case 3:
			JSPrintMsg("<table width=100%><tr align=left><td><b>Peer-reviewed publications in National Journals</td></tr> <tr height=2><td bgcolor=#003399></td></tr></table>");
			break;
			case 4:
			JSPrintMsg("<table width=100%><tr align=left><td><b>Peer-reviewed talks in international events </td></tr> <tr height=2><td bgcolor=#003399></td></tr></table>");
			break;
			case 5:
			JSPrintMsg("<table width=100%><tr align=left><td><b>Peer-reviewed talks in national events</td></tr> <tr height=2><td bgcolor=#003399></td></tr></table>");
			break;
			case 6:
			JSPrintMsg("<table width=100%><tr align=left><td><b>Various talks and publications</td></tr> <tr height=2><td bgcolor=#003399></td></tr></table>");
			break;
			case 7:
			JSPrintMsg("<table width=100%><tr align=left><td><b>Contractual reports</td></tr> <tr height=2><td bgcolor=#003399></td></tr></table>");
			break;
			case 8:
			JSPrintMsg("<table width=100%><tr align=left><td><b>Internal reports</td></tr> <tr height=2><td bgcolor=#003399></td></tr></table>");
			break;
			case 9:
			JSPrintMsg("<table width=100%><tr align=left><td><b>Thesis and academic qualification</td></tr> <tr height=2><td bgcolor=#003399></td></tr></table>");
			break;
		}
	}
	...
	JSPrintMsg("<table width=100%> <tr align='left' valign='top'><td width=12% rowspan=3 align='center'><b>[$titre]</b><br /><br />");
		// link to cart in standard view
		JSPrintMsg("</td><td width=88%>  <a href='$lien'>$row[title]</a>  $docs</td></tr><tr align='left'><td width=88%>");
  		// Search for authors
		  JSPrintMsg("  <div class='authors'>");
		  printAuthors($id);
		  JSPrintMsg("  </div></td></tr>");
		//printName($name,$last,$url);
	JSPrintMsg("<tr align='left'><td width=88%>");
	...
	switch($row['entry'])
	{
		...
		case "Article":
			JSPrintMsg("$row[journal]");
			if (!empty($row[publisher])) JSPrintMsg(" $row[publisher]");	// rajouté le 25/04/2006
			if (!empty($row[pages])) JSPrintMsg(" pages $row[pages]");	
			if (!empty($row[volume])) JSPrintMsg(" vol. $row[volume]");	
			if (!empty($row[number])) JSPrintMsg(" num. $row[number]");	
			JSPrintMsg(" - $row[month] $row[year]");
			//if (!empty($row[note])) echo "<br>$row[note]";	
			if (!empty($row[issn])) JSPrintMsg("<br><small><i>issn: $row[issn]</i></small>");	
			if (!empty($row[doi])) JSPrintMsg("&nbsp;&nbsp;<small><i>doi: <a href=\"http://dx.doi.org/".$row[doi]."\">$row[doi]</a></i></small>");	
			break;
		case "InProceedings":
			JSPrintMsg(" $row[booktitle]");
			if (!empty($row[publisher])) JSPrintMsg(" $row[publisher]");	
			if (!empty($row[series])) JSPrintMsg(" $row[series]");	
			if (!empty($row[pages])) JSPrintMsg(" pages $row[pages]");	
			if (!empty($row[volume])) JSPrintMsg(" vol. $row[volume]");	
			if (!empty($row[number])) JSPrintMsg(" num. $row[number]");	
			if (!empty($row[editor])) JSPrintMsg(" $row[editor]");	
			if (!empty($row[organization])) JSPrintMsg(" $row[organization]");	
			if (!empty($row[address])) JSPrintMsg(" $row[address]");	
			JSPrintMsg(" - $row[month] $row[year]");
			//if (!empty($row[note])) echo "<br>$row[note]";	
			if (!empty($row[issn])) JSPrintMsg("<br><small><i>issn: $row[issn]</i></small>");	
			if (!empty($row[doi])) JSPrintMsg("&nbsp;&nbsp;<small><i>doi: <a href=\"http://dx.doi.org/".$row[doi]."\">$row[doi]</a></i></small>");	
			break;
		case "InBook":
			JSPrintMsg(" $row[publisher], chap. $row[chapter], pages $row[pages]");
			if (!empty($row[series])) JSPrintMsg(" $row[series]");	
			if (!empty($row[address])) JSPrintMsg(" $row[address]");	
			if (!empty($row[volume])) JSPrintMsg(" vol. $row[volume]");	
			if (!empty($row[number])) JSPrintMsg(" num. $row[number]");	
			if (!empty($row[edition])) JSPrintMsg(" $row[edition]");	
			if (!empty($row[type])) JSPrintMsg(" $row[type]");	
			if (!empty($row[proceedings])) JSPrintMsg(" $row[proceedings]");	
			JSPrintMsg(" - $row[month] $row[year]");
			//if (!empty($row[note])) echo "<br>$row[note]";	
			break;	
		case "Book":
		...
		case "PhdThesis":
		case "MastersThesis":
			if (!empty($row[type])) JSPrintMsg("$row[type]<br>");	
			JSPrintMsg("$row[school] - ");
			if (!empty($row[address])) JSPrintMsg("$row[address] - ");	
			JSPrintMsg("$row[month] $row[year]");
			//if (!empty($row[note])) echo "<br>$row[note]";	
			break;
		case "TechReport":
			JSPrintMsg(" $row[institution]");
			if (!empty($row[address])) JSPrintMsg(" $row[address]");	
			if (!empty($row[number])) JSPrintMsg(" num. $row[number]");	
			if (!empty($row[type])) JSPrintMsg(" $row[type]");	
			JSPrintMsg(" - $row[month] $row[year]");
		...
		case "Misc":
			if (!empty($row[type])) JSPrintMsg("$row[type] - ");	
			if (!empty($row[howpublished])) JSPrintMsg(" $row[howpublished]");	
			JSPrintMsg(" - $row[month] $row[year]");
			//if (!empty($row[note])) echo "<br>$row[note]";	
			break;
		case "Booklet":
		...
		case "InCollection":
			...
			if (!empty($row[number])) JSPrintMsg(" num. $row[number]");	
			if (!empty($row[series])) JSPrintMsg(" $row[series]");	
			if (!empty($row[chapter])) JSPrintMsg(" chap. $row[chapter]");	
			if (!empty($row[address])) JSPrintMsg(" $row[address]");	
			if (!empty($row[edition])) JSPrintMsg(" $row[edition]");	
			if (!empty($row[type])) JSPrintMsg(" $row[type]");	
			JSPrintMsg(" - $row[month] $row[year]");
...
function displayResults($result, $option, $frame=true)
{
  ...
  if ($option["display"]=="gallery")
    JSPrintMsg("<div class='results gallery'>");
  else
    if ($option["display"]=="bibtex")
      JSPrintMsg("<div class='results bibtex'>");
    else
      JSPrintMsg("<div class='results list'>");
  ...
  while ($result && $row=mysql_fetch_array($result))
      ...
      if ($row["year"] != $prevYear)
	{
	  ...
	  if (($option["display"]=="bibtex") || ($option["display"]=="xml"))
	    {
	      if ($addHR)		
		JSPrintMsg(" <hr style=\"height: 5px;\"/>");
	    }
	  else
	    {
	      ...
	      if ($addHR)
		JSPrintMsg(" <div class='interYear'>&nbsp;</div>");
	  JSPrintMsg("<div class='year'>$txtPubli $txtIn $prevYear </div>");
	    }
	  $addHR = false;
	}
...
//	      printPubliLine($row);
  ...
  JSPrintMsg("<br style='clear:both'/>");
}
